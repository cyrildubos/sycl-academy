...
int main() {
  ...
  queue.memcpy(a_device, a, n * sizeof(float)).wait();
  queue.memcpy(b_device, b, n * sizeof(float)).wait();

  queue
      .parallel_for(
          sycl::range{n},
          [=](sycl::id<1> id) { c_device[id] = a_device[id] + b_device[id]; })
      .wait();

  queue.memcpy(c, c_device, n * sizeof(float)).wait();
